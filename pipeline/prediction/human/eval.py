...
def evaluate_translations(
    ...
    df = df[df["src"].str.strip().astype(bool) & df["mt"].str.strip().astype(bool)]
    filtered_len = len(df)
    ...
    data = [
        {
            ...
            "mt": str(row["mt"]).strip(),
            "ref": (str(row.get("ref", "")).strip() if "ref" in row else ""),
        }
    ...
    for evaluation_function, name in evaluation_functions:
        ...
        try:
            ...
            results = evaluation_function(data)
