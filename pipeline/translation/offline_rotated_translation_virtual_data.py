...
from pipeline.models.nllb_batch import translate_batch_with_timing as nllb_translate
from pipeline.models.mixtral import translate_batch_with_timing as mixtral_translate
from pipeline.models.qwen import translate_batch_with_timing as qwen_translate
...
def save_iteration(
    ...
    output_file = os.path.join(output_dir, f"offline_{iteration}_mr1.json")
...
def main():
    try:
        ...
        with open("outputs/batch_output/offline_0.json", "r", encoding="utf-8") as f:
            items = json.load(f)

        model_sequence = [
            (gemma_translate, "Gemma3", 32),
            (gemma_translate, "Gemma3", 32),
            (qwen_translate, "Qwen", 24),
            (qwen_translate, "Qwen", 24),
            (nllb_translate, "NLLB", 1024),
            (nllb_translate, "NLLB", 1024),
        ]
