...
def save_iteration(
    ...
    output_file = os.path.join(output_dir, f"offline_{iteration}_lowdiv_dir.json")
...
def main():
    try:
        ...
        # Load initial data
        with open(
            "outputs/batch_output/offline_11_lowdiv_dir.json", "r", encoding="utf-8"
        ) as f:
            items = json.load(f)
        ...
        model_group = (gemma_translate, "Gemma3", 32)
        ...
        for iteration in range(12, max_iterations + 1):
            try:
                logging.info(f"\nStarting iteration {iteration}")

                items = get_correct_sequence_triplets(
                    items, triplets_dict, iteration, is_direct=True
                )
...
if __name__ == "__main__":
    try:
        ...
        try:
            ...
            sys.stderr.write("rotated_translation_main() failed:\n")
