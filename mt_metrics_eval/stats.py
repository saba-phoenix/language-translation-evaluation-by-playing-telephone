...
class Correlation:
    ...
    def KendallWithTiesOpt(self, average_by="none", variant="acc23", sample_rate=0.1):
        """Kendall with ties, with optimized threshold."""
        cf = self.AverageCorrelation(
            KendallWithTiesOpt, average_by, variant=variant, sample_rate=sample_rate
        )
        return cf(self.gold_scores, self.metric_scores)
...
def filter_gold_nones(gold: ArrayLike, model: ArrayLike) -> tuple[ArrayLike, ArrayLike]:
    ...
    def Corr(self, gold_vect: ArrayLike, metric_vect: ArrayLike):
        ...
        if self._corr_fcn is KendallWithTiesOpt:
            corr, _, _ = KendallWithTiesOpt(
                gold_vect,
                metric_vect,
                num_sys=self._num_sys,
                average_by=self._average_by,
                **self._corr_fcn_args,
            )
        ...
        # PairwiseConfidenceError does its own averaging.
            ...
            corr = PairwiseConfidenceError(
                ...
                metric_vect,
                num_sys=self._num_sys,
                filter_nones=self._filter_nones,
                **self._corr_fcn_args,
            )
...
class KendallPreproc:
    ...
    def __init__(self, y):
        ...
        self.y_cumsum = np.r_[True, y_sorted[1:] != y_sorted[:-1]].cumsum(dtype=np.intp)
...
def _MatrixSufficientStatistics(
    ...
    # be an integer.
    con = int(((x_diffs > 0) & (y_diffs > 0) | (x_diffs < 0) & (y_diffs < 0)).sum() / 2)
...
def KendallWithTiesOpt(
    ...
    opt_result = tau_optimization.tau_optimization(metric, gold, tau_fn, sample_rate)
...
def PairwiseConfidenceError(
    ...
    if filter_nones:
        gold_scores, metric_scores = filter_gold_nones(gold_scores, metric_scores)
    ...
    def _Corr(mscores):
        with warnings.catch_warnings():
            ...
            if corr_fcn is KendallWithTiesOpt:
                ...
                vals = [
                    corr_fcn(gold[b:e], mscores[b:e], **corr_fcn_args)[0]
                    for b, e in bounds
                ]
...
def PairwisePermutationSigDiff(
    ...
    def _Corr(preprocs):
        vals = [
            KendallVariants(None, None, variant, g, 0, m)[0]
    ...
    def _SubsampleSizes(self, proportions, size, bin_sizes):
        ...
        if (bin_sizes < sizes).any():
            ...
            realloc_sizes = self._subsample_sizes(
                proportions_to_reallocate, mass_to_reallocate, bin_sizes_to_reallocate
            )
