# copied from https://github.com/google-research/mt-metrics-eval
# coding=utf-8
...
WMT20 = MetaInfo(
    "ref", {"sys": "wmt-z", "doc": "wmt-raw", "seg": "wmt-raw"}, set(), set()
)
WMT21_PRIMARIES = {
    "bleurt-20",
    "COMET-MQM_2021",
...
WMT21 = MetaInfo("refA", {"sys": "wmt-z", "seg": "wmt-raw"}, set(), WMT21_PRIMARIES)
...
WMT22_PRIMARIES = {
    ...
    "COMET-QE",
    "f200spBLEU",
    "HuaweiTSC_EE_BERTScore_0.3_With_Human",
    "HWTSC-Teacher-Sim",
    "MATESE-QE",
    "MATESE",
    "MEE4",
...
WMT22 = MetaInfo(
    "refA", {"sys": "mqm", "domain": "mqm", "seg": "mqm"}, set(), WMT22_PRIMARIES
)
WMT22_DA = MetaInfo(
    "refA", {"sys": "wmt", "domain": "wmt", "seg": "wmt"}, set(), WMT22_PRIMARIES
)
WMT22_DA_NODOMAIN = MetaInfo(
    "refA", {"sys": "wmt", "seg": "wmt"}, set(), WMT22_PRIMARIES
)
...
WMT22_NODOMAIN = MetaInfo(
    "refA", {"sys": "wmt-appraise", "seg": "wmt-appraise"}, set(), WMT22_PRIMARIES
)
...
WMT24_PRIMARIES = {
    ...
    "gemba_esa",
    "MEE4",
    "metametrics_mt_mqm_hybrid_kendall",
    "metametrics_mt_mqm_qe_kendall.seg.s",
    "MetricX-24-Hybrid",
...
DATA = {
    ...
    "wmt20": {
        "cs-en": dataclasses.replace(
            WMT20, outlier_systems={"zlabs-nlp.1149", "CUNI-DocTransformer.1457"}
        ),
        "de-en": dataclasses.replace(
            WMT20,
            outlier_systems={"yolo.1052", "zlabs-nlp.1153", "WMTBiomedBaseline.387"},
        ),
        "en-cs": dataclasses.replace(
            WMT20, outlier_systems={"zlabs-nlp.1151", "Online-G.1555"}
        ),
        "en-de": dataclasses.replace(
            WMT20,
            outlier_systems={"zlabs-nlp.179", "WMTBiomedBaseline.388", "Online-G.1556"},
        ),
        "en-iu": dataclasses.replace(
            WMT20, outlier_systems={"UEDIN.1281", "OPPO.722", "UQAM_TanLe.521"}
        ),
        "en-ja": dataclasses.replace(
            WMT20, outlier_systems={"Online-G.1557", "SJTU-NICT.370"}
        ),
        "en-pl": dataclasses.replace(
            WMT20, outlier_systems={"Online-Z.1634", "zlabs-nlp.180", "Online-A.1576"}
        ),
        "en-ru": WMT20,
        "en-ta": dataclasses.replace(
            WMT20, outlier_systems={"TALP_UPC.1049", "SJTU-NICT.386", "Online-G.1561"}
        ),
        "en-zh": WMT20,
        "iu-en": dataclasses.replace(
            WMT20, outlier_systems={"NiuTrans.1206", "Facebook_AI.729"}
        ),
        "ja-en": dataclasses.replace(
            WMT20, outlier_systems={"Online-G.1564", "zlabs-nlp.66", "Online-Z.1640"}
        ),
        "km-en": WMT20,
        "pl-en": dataclasses.replace(WMT20, outlier_systems={"zlabs-nlp.1162"}),
        "ps-en": WMT20,
        "ru-en": dataclasses.replace(WMT20, outlier_systems={"zlabs-nlp.1164"}),
        "ta-en": dataclasses.replace(
            WMT20, outlier_systems={"Online-G.1568", "TALP_UPC.192"}
        ),
        "zh-en": dataclasses.replace(WMT20, outlier_systems={"WMTBiomedBaseline.183"}),
    },
    "wmt19": {
        "de-cs": WMT19,
        "de-en": WMT19,
